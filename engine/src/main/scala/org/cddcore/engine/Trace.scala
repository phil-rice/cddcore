...
class NestedTraceBuilder(val params: List[Any], engine: Engine[_], items: List[TraceItem], val parent: TraceBuilder) extends TraceBuilder(items) {
  override def finished(conclusion: Conclusion,result: Any) = parent.copyWithNewItem(new TraceItem(engine, params, items, conclusion, result, System.nanoTime() - startTime))
...
case class TraceItem(engine: Engine[_], params: List[Any], nested: List[TraceItem],conclusion: Conclusion,  result: Any, took: Long) {
