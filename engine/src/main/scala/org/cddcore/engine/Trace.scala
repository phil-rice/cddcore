...
class NestedTraceBuilder(val params: List[Any], engine: SimpleEngine[_], items: List[TraceItem], val parent: TraceBuilder) extends TraceBuilder(items) {
...
class TraceBuilder(val items: List[TraceItem]) {
  ...
  def nest(e: SimpleEngine[_], params: List[Any]) = new NestedTraceBuilder(params, e, List(), this)
...
case class TraceItem(engine: SimpleEngine[_], params: List[Any], nested: List[TraceItem], conclusion: Conclusion, result: ROrException[Any], took: Long) {
