...
class NestedTraceBuilder(val params: List[Any], engine: Engine, items: List[TraceItem], val parent: TraceBuilder) extends TraceBuilder(items) {
...
class TraceBuilder(val items: List[TraceItem]) {
  ...
  def nest(e: Engine, params: List[Any]) = new NestedTraceBuilder(params, e, List(), this)
...
case class TraceItem(engine: Engine, params: List[Any], nested: List[TraceItem], conclusion: Conclusion, result: ROrException[Any], took: Long) {
