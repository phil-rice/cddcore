...
class RequirementHolderForTest(name: String, val children: List[Requirement]) extends RequirementForTest(name) with ReportableHolder {
  override def toString() = name
}
...
//    def check(holder: ReportableHolder, expected: String*) = {
...
//  def checkPrinter(holder: ReportableHolder, expected: String) = assertEquals(ReqPrintContext(1, expected,new NoNameForRequirement), holder.fold(ReqPrintContext())(new SimpleRequirementsPrinter))
//
