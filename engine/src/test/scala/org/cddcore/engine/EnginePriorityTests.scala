  ...
  "An Engine" should "allow the priority to set the rules order" in {
    def build(a: Int, b: Int, c: Int) =
      builder.code((x: String) => "U").
        useCase("U").
        scenario("A").because("A").expectedAndCode("FromA").priority(a).
        scenario("B").because("B").expectedAndCode("FromB").priority(b).
        scenario("C").because("C").expectedAndCode("FromC").priority(c).
        build
